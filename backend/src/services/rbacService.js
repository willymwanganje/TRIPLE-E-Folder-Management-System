...
export async function getUserPermissions(userId) {
  ...
  const names = new Set();
  for (const row of user.role?.permissions || []) names.add(row.permission.name);
  for (const row of user.permissions || []) {
    if (row.allowed) names.add(row.permission.name);
    else names.delete(row.permission.name);
  }
  return [...names];
}
...
export function isPrivileged(user) {
  return Boolean(user?.isSuperAdmin || user?.role?.name === 'Admin');
}
...
export function canAccessResource(user, resource, ownerField) {
  ...
  // Owners retain access to their own resources.
  if (ownerField && resource[ownerField] === user.id) return true;
  return false;
}

export function isOwner(userId, resource, ownerField) {
